...
export default function Header() {
  ...
  const navItems = [
    ...
    { path: "/week-plan", label: "Week Plan", icon: Calendar, active: location === "/week-plan" },
  ...
  return (
    <header className="bg-white shadow-xl sticky top-0 z-50" style={{ borderBottom: '4px solid var(--theme-200)' }}>
      <div className="max-w-md mx-auto px-4 py-4">
        {/* Header Content */}
        <div className="flex items-center justify-between mb-4">
          {/* Logo */}
          <Link href="/" className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-2xl flex items-center justify-center shadow-lg transform rotate-12" style={{ backgroundColor: 'var(--theme-400)' }}>
              <Heart className="w-5 h-5 text-white" />
            </div>
            <h1 className="text-2xl font-handwritten font-bold transform -rotate-2" style={{ color: 'var(--theme-900)' }}>
              Meal planner
            </h1>
            <img 
              src={catMascotImage} 
          ...
          <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
            <SheetTrigger asChild>
              <Button variant="ghost" size="sm" className="md:hidden p-2 rounded-2xl hover:shadow-md transition-all duration-200"
                style={{ '--hover-bg': 'var(--theme-50)' } as React.CSSProperties}
                onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'var(--theme-50)'}
                onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
              >
                <Menu className="w-5 h-5" style={{ color: 'var(--theme-700)' }} />
              </Button>
            </SheetTrigger>
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-handwritten font-bold" style={{ color: 'var(--theme-900)' }}>Menu</h2>
              </div>
